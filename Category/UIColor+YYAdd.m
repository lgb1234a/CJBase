...
+ (UIColor *)yy_colorWithHue:(CGFloat)hue
...
+ (UIColor *)yy_colorWithCyan:(CGFloat)cyan
...
+ (UIColor *)yy_colorWithRGB:(uint32_t)rgbValue {
...
+ (UIColor *)yy_colorWithRGBA:(uint32_t)rgbaValue {
...
+ (UIColor *)yy_colorWithRGB:(uint32_t)rgbValue alpha:(CGFloat)alpha {
    return [UIColor colorWithRed:((rgbValue & 0xFF0000) >> 16) / 255.0f
                           green:((rgbValue & 0xFF00) >> 8) / 255.0f
                            blue:(rgbValue & 0xFF) / 255.0f
                           alpha:alpha];
...
static inline NSString *stringByTrim(NSString *str) {
...
static BOOL hexStrToRGBA(NSString *str,
    ...
    str = [stringByTrim(str) uppercaseString];
    if ([str hasPrefix:@"#"]) {
...
+ (instancetype)yy_colorWithHexString:(NSString *)hexStr {
    CGFloat r, g, b, a;
...
- (NSString *)yy_hexStringWithAlpha {
...
- (NSString *)hexStringWithAlpha:(BOOL)withAlpha {
    ...
    if (hex && withAlpha) {
        hex = [hex stringByAppendingFormat:@"%02lx",
               (unsigned long)(self.yy_alpha * 255.0 + 0.5)];
    }
    return hex;
}

- (UIColor *)yy_colorByAddColor:(UIColor *)add blendMode:(CGBlendMode)blendMode {
...
- (UIColor *)yy_colorByChangeHue:(CGFloat)h saturation:(CGFloat)s brightness:(CGFloat)b alpha:(CGFloat)a {
    CGFloat hh, ss, bb, aa;
...
- (CGFloat)yy_saturation {
    CGFloat h, s = 0, b, a;
    [self getHue:&h saturation:&s brightness:&b alpha:&a];
    return s;
}

- (CGFloat)yy_brightness {
    CGFloat h, s, b = 0, a;
    [self getHue:&h saturation:&s brightness:&b alpha:&a];
    return b;
}

- (CGColorSpaceModel)yy_colorSpaceModel {
    return CGColorSpaceGetModel(CGColorGetColorSpace(self.CGColor));
}

- (NSString *)yy_colorSpaceString {
